            ...
            <div class="col-md-12">
                  ...
                  <button type="button" class="list-group-item list-group-item-action" @click = "addItem(item,index)" :class="{active: item.active}">{{item.itemName}}<span class="badge badge-default badge-pill">{{item.itemSaleStatus}}</span></button>
                    ...
                    <tr>
                      ...
                      <tr v-for = "(carts, index) in itemLists.items">
                        ...
                        <div class="col-md-4">
                          ...
                          <input type="number" min="1"  id = "insms" class = "text-center"name="" :value="carts['qty']" v-model="index.value" @input = "getdata(carts['item']['id'])" onkeypress="return isNumber(event)">
...
axios.get('/riv-admin/addcart/'+ item.id).then(function(response){


Vue.set(vm.$data, 'itemLists', response.data)
